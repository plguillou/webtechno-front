...
export default function BookRequestModal({show, onHide, selectedHouseId}) {
    ...
    const handleSendClick = () => {
        ...
        addNewBooking(startDate, endDate, selectedHouseId, chosenHouse?.id !== 0 && chosenHouse.id).then(r => {
            // console.log(r)
            setIsSending(false)
            onHide()
        })
    ...
    >
        ...
        <Modal.Body>
            ...
            <div>
                ...
                <div className={"text-muted"}>Vous pourrez modifier ce choix par la suite</div>
                <DropdownButton id="dropdown-basic-button" title={chosenHouse ? chosenHouse.title : "Votre résidence"}
                                className={"pt-2 px-2"}>
                    {
                        [{title: "Aucune résidence ", id: 0}, ...userHouses].map((elem, i) => (
                            <>

                                <Dropdown.Item key={i}
                                               onSelect={() => setChosenHouse({title: elem.title, id: elem.id})}>
                                    {elem.title}
                                </Dropdown.Item>
                                {elem.id === 0 && <Dropdown.Divider/>}
                            </>
                        ))
                    }
                </DropdownButton>
